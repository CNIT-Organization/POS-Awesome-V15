<template>
	<v-container class="pa-4 invoice-details-page" fluid>
		<v-row class="mb-2" align="center">
			<v-col cols="12" md="8">
				<div class="text-h6 font-weight-bold">{{ __("Invoice") }}: {{ invoiceName }}</div>
				<div class="text-body-2 text-medium-emphasis">{{ __("Recipe usage and item consumption details") }}</div>
			</v-col>
		...
		<v-row dense class="mb-2">
			<v-col cols="12" md="4">
				<v-card class="metric-card pa-3">
					<div class="text-caption">{{ __("Grand Total") }}</div>
					<div class="text-h6">{{ doc?.grand_total }} {{ doc?.currency }}</div>
				</v-card>
			</v-col>
			<v-col cols="12" md="4">
				<v-card class="metric-card pa-3">
					<div class="text-caption">{{ __("Sold Lines") }}</div>
					<div class="text-h6">{{ soldItems.length }}</div>
				</v-card>
			</v-col>
			<v-col cols="12" md="4">
				<v-card class="metric-card pa-3">
					<div class="text-caption">{{ __("Consumed Components") }}</div>
					<div class="text-h6">{{ packedItems.length }}</div>
				</v-card>
			</v-col>
		</v-row>
		...
		<v-row dense class="mb-2">
			<v-col cols="12" md="6">
				<v-card class="pos-themed-card pa-3 info-card">
					<div class="text-subtitle-2 mb-1">{{ __("Header") }}</div>
					<div class="text-body-2"><strong>{{ __("Date") }}:</strong> {{ doc?.posting_date }}</div>
					<div class="text-body-2"><strong>{{ __("Customer") }}:</strong> {{ doc?.customer_name }}</div>
				</v-card>
			</v-col>
			<v-col cols="12" md="6">
				<v-card class="pos-themed-card pa-3 info-card">
					<div class="text-subtitle-2 mb-1">{{ __("Meta") }}</div>
		...
		<v-row dense>
			<v-col cols="12" md="6">
				<v-card class="table-card pa-2">
					<div class="text-subtitle-1 mb-2">{{ __("Sold Items") }}</div>
					<v-data-table :headers="soldHeaders" :items="soldItems" item-key="name" density="comfortable" />
				</v-card>
			</v-col>
			<v-col cols="12" md="6">
				<v-card class="table-card pa-2">
					<div class="text-subtitle-1 mb-2">{{ __("Consumed Components (Recipes)") }}</div>
					<v-data-table :headers="packedHeaders" :items="packedItems" item-key="id" density="comfortable" />
				</v-card>
			</v-col>
		</v-row>
	</v-container>
...
.invoice-details-page {
	--border: rgba(var(--v-theme-on-surface), 0.1);
}
...
.table-card :deep(.v-data-table-header th) {
